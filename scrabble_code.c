...
char t[15][15][5][7];           //array to display the board
int tot1=0;                     //first player score
int tot2=0;                     //second player score
int tcount=0;                   //tiles count

struct node   //contains details present in letters1.csv
{
...
typedef struct tnode  //binary tree with nodes being the words player forms.
 {
...
char ply[10];         //randomly generated letters the player should use to form words
char arr[10];


tnode * createNode(char *word)  //creates new node for the word formed by player.
{
        tnode *newnode;
...
void insert_tree(char *word)  //inserts newnode formed into the binary tree at its respective position maintaing the binary tree constraints.
{
        ...
        for (current = head; current !=NULL;current = (res > 0) ? current->right : current->left)
        {
...
int search_tree(char *str)  //checks whether the new word entered is already formed before by traversing through the binary tree.
{
        tnode *temp = NULL;
        int flag = 0, res = 0;
        if (head == NULL) {
...
void create_list(struct node**f)   //creates list containing details present in letters1.csv
{
  char ch[20];
  int l,i,ctr,scr;
  char *tok;
...
void random(int x)      //randomly generates x number of letters
{
    int i;
    srand(time(NULL)+i);
    for(i=0;i<x;i++)
        ...
        label:
            {
                ...
                if(n==0)           //when count of randomly generated letter becomes zero
                {
                    if(tcount==100)
                    {
...
int traversal(struct node **p,int c)  //checks whether randomly generated letter is present in the heap if tiles
{
   struct node *q,*prev;
    q=*p;
...
int search_score(struct node **p,int c)  //search the score of a particular letter
{
   struct node *q,*prev;
    q=*p;
...
void insert(char *a,int x)   //inserts letter x into array a
{
    int i=0;
...
int check(char *in)              //checks whether the word is valid or not
{
    char ch[100];
    FILE *fp;
...
int search_places_hor(int r,int c,char *w,int x)       //searches if the letters entered by the user can be placed on the board after seeing the words around the index mentioned horizontally
{
    ...
    //newword will have the word that will be formed when the letters entered by the user is places on the board by adding the words left and wight of it if present

    //eg: if "as" is at col.index 9,10 and player enters letter "h" at col.index 8
    // left_word will be NULL
    // arr will be "h"
    // right_word will be "as"
    // new_word will be "has"

    int lscr=0,rscr=0;           //left word score and right word score
   ...
   // printf("new word=%s\n",new_word);
    ...
    if(check(new_word) && search_tree(new_word))         //checks if valid and not repeated
    {
        //printf("valid word\n");
        if(x==1)
            tot1=tot1+lscr+rscr;
...
int search_places_ver(int r,int c,char *w,int x)   //searches if the letters entered by the user can be placed on the board after seeing the words around the index mentioned vertically
{
    int i=0,j=0,index=0,l=0;
